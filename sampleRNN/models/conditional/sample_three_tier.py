...
def get_args():
    parser = argparse.ArgumentParser(description='sample_three_tier.py')
    parser.add_argument('--dir', help='Experiment dir',
...
DIR = args.dir
with open(os.path.join(DIR, 'args.pkl'), 'rb') as f:
    list_args = pkl.load(f)
"""
['models/three_tier/three_tier.py', '--exp', 'DIM', '--seq_len', '512',
'--big_frame_size', '8', '--frame_size', '2', '--emb_size', '256',
'--skip_conn', 'False', '--dim', '512', '--n_rnn', '3', '--rnn_type', 'GRU',
'--q_levels', '256', '--q_type', 'linear', '--batch_size', '128',
'--weight_norm', 'True', '--learn_h0', 'True',
'--which_set', 'ONOM', '--resume']
"""
SEQ_LEN = 512 #args.seq_len # How many samples to include in each truncated BPTT pass
BIG_FRAME_SIZE = 8 #args.big_frame_size # how many samples per big frame
FRAME_SIZE = 2 #args.frame_size # How many samples per frame
OVERLAP = BIG_FRAME_SIZE
WEIGHT_NORM = True #args.weight_norm
EMB_SIZE = 256 #args.emb_size
SKIP_CONN = False #args.skip_conn
DIM = 512 #args.dim # Model dimensionality.
BIG_DIM = DIM # Dimensionality for the slowest level.
N_RNN = 3 #args.n_rnn # How many RNNs to stack in the frame-level model
N_BIG_RNN = N_RNN # how many RNNs to stack in the big-frame-level model
RNN_TYPE = 'GRU' #args.rnn_type
H0_MULT = 2 if RNN_TYPE == 'LSTM' else 1
LEARN_H0 = True #args.learn_h0
Q_LEVELS = 256 #args.q_levels # How many levels to use when discretizing samples. e.g. 256 = 8-bit scalar quantization
Q_TYPE = 'linear' #args.q_type # log- or linear-scale
WHICH_SET = 'ONOM' #args.which_set
...
# TODO: 90*60 -> 180*60
PRINT_TIME = 60*180 # Print cost, generate samples, save model checkpoint every N seconds.
...
FOLDER_PREFIX = DIR #os.path.join(RESULTS_DIR, tag)
...
def generate_and_save_samples(tag, seed=None):
    def write_audio_file(name, data):
        ...
        scipy.io.wavfile.write(
                    #os.path.join(BEST_PATH, name+'.wav'),
                    name+'.wav',
                    BITRATE,
                    data)
    ...
    N_SECS = 20
    LENGTH = N_SECS*BITRATE if not DEBUG else 100

    samples = numpy.zeros((N_SEQS, LENGTH), dtype='int32')
    samples[:, :BIG_FRAME_SIZE] = Q_ZERO
    if seed is not None:
        samples[:, BIG_FRAME_SIZE:BIG_FRAME_SIZE+len(seed)] = seed
    ...
    for t in xrange(BIG_FRAME_SIZE, LENGTH):
        ...
        sample_t = sample_level_generate_fn(
            frame_level_outputs[:, t % FRAME_SIZE],
            samples[:, t-FRAME_SIZE:t]
        )
        if seed is not None:
            if t > BIG_FRAME_SIZE + len(seed):
                samples[:, t] = sample_t
        else:
            samples[:, t] = sample_t
...
for seed_ind in [0, 32, 64, 96, 128, 160, 192, 224, 240, 255][::-1]:
    ...
    for div in [1, 2, 4]:
        print "\tdiv: ", div
        # Just seed
        seed_len = len(selected_seed[0])//div
        seed_len = seed_len//BIG_FRAME_SIZE
        seed_len = seed_len * BIG_FRAME_SIZE
        seed = selected_seed[0, :seed_len]

        for silence_len in [0, 4800, 8000, 16000]:
            silence_len = silence_len//BIG_FRAME_SIZE
            silence_len = silence_len * BIG_FRAME_SIZE
            print "\t\tAdded silence:", silence_len
            seed_with_silence = Q_ZERO * numpy.ones(shape=(seed_len+silence_len,), dtype='int32')
            seed_with_silence[:seed_len] = seed

            save_audio_file(
                'seed{}_div{}_sil{}.wav'.format(seed_ind, div, silence_len),
                seed_with_silence)
            generate_and_save_samples(
                'seeded_{}_div_{}_silence_{}_samples'.format(seed_ind, div, silence_len),
                seed_with_silence)
