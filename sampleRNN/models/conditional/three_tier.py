...
def generate_and_save_samples(
    ...
    if npy_address is not None:
        ...
        test_feats_mask = np.ones(test_feats.shape[0:2], dtype='float32')
        ...
        if features_mask is not None:
            test_feats_mask = features_mask.astype('float32').transpose(1,0)
        else:
            test_feats_mask = np.ones(test_feats.shape[0:2], dtype='float32')
    ...
    for i in xrange(n_seqs):
        cut_point = int(test_feats_mask[i].sum())  # number of unmasked frames
        cut_point *= 80
